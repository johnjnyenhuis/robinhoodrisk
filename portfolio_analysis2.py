...
def load_portfolio(file_path):
    ...
    with open(file_path, 'r') as file:
        next(file)  # Skip the header row (ticker,shares)
        for line in file:
            ticker, shares = line.strip().split(',')
            portfolio[ticker] = float(shares)  # Convert shares to integer
    return portfolio
...
class PortfolioRiskAnalyzer:
    def __init__(self, portfolio, risk_free_rate=0.04):  # 4% annual risk-free rate
        ...
        # define colors for stocks and portfolio
        self.colors = {
            'SPY': 'blue',
            'QQQ': 'red',
            'GLD': 'purple'
        }
        self.portfolio_color = 'black'
...
# Load portfolio from CSV and run analysis
portfolio = load_portfolio('portfolio.csv')
analyzer = PortfolioRiskAnalyzer(portfolio)
analyzer.analyze()